...
#define RSOSUIPHONETEXTFIELD_KEYPRESSED_BACKSPACE        1000

@implementation RSOSUIPhoneTextField
...
- (BOOL) isPhonenumberForUS{
    return [self.regionCode caseInsensitiveCompare:@"US"] == NSOrderedSame;
}
...
- (NSString *) beautifyPhoneNumber :(NSString *)szOriginalPhoneNumber isBackspace:(BOOL) isBackspace{
    szOriginalPhoneNumber = [self stripNonnumericsFromNSString:szOriginalPhoneNumber];
...
- (IBAction)onEditingChanged:(id)sender {
    ...
    if (tag == RSOSUIPHONETEXTFIELD_KEYPRESSED_BACKSPACE) isBackspace = YES;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (string.length == 0){
        [self setTag:RSOSUIPHONETEXTFIELD_KEYPRESSED_BACKSPACE];
    }
    else{
        [self setTag:0];
    }
